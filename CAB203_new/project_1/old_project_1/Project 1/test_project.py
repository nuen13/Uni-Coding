...
def testPotentialReferees_3():
   assert P.potentialReferees( os.path.join(scriptDirectory, 'referees2.csv'), 'Gong Gong', 'Jobu Tupaki') == { 'Waymond', 'Deirdre' }
...
def testModulesOK():
   allowed_modules = { 'typing.re', 'digraphs', 'project', 'typing', 'csv', 'graphs', 'collections.abc', 'typing.io', '_csv', 're' }
   if not (modules_after - modules_before).issubset(set(allowed_modules)):
      warnings.warn(f"You have imported one or more modules which are not on the list of pre-approved modules.  Please check that it is needed and gain approval from the unit coordinator.\nImported modules: {modules_after - modules_before}")
